...
for (replicate in 0:(num_replicates - 1)) { # replicates 0-99
  ...
  gen_out1 <- read.table(paste0("Outputs/Batch9_Sim1_Land1_Rep", replicate, "_Genetics.txt"), header = TRUE)
  # merge
...
t<-read.table("Outputs/Batch9_Sim1_Land1_Rep0_Inds.txt")
# Loop through each replicate
for (replicate in 0:(num_replicates - 1)) { # replicates 0-99
  # Read in genetic output
  ind_out1 <- read.table(paste0("Outputs/Batch9_Sim1_Land1_Rep", replicate, "_Inds.txt"), header = TRUE)
  # merge
...
# Load data
load("data/gen_ind_out_s9.RData")
gen_ind_out_s9[121:240] <- NULL
save(gen_ind_out_s9, file = "data/gen_ind_out_s9.RData")
# subset data and drop all rows with PatchID = 0 (currently not in a population)
filtered_data_s9 <- gen_ind_out_s9[!(gen_ind_out_s9$PatchID==0),]
...
for (year in c(20, 210)) {
  for (rep in 0:(num_replicates - 1)) { # replicates 0-9
    ...
    filtered_data_year_rep <- filtered_data_s9[!is.na(filtered_data_s9$Year) & 
                                                  !is.na(filtered_data_s9$Rep) &
                                                  filtered_data_s9$Year == year & 
                                                  filtered_data_s9$Rep == rep, ]
...
gen_sim_df20_s9 <- melted_df_list_20 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df20_s9, file = "data/s9_gen_sim_df20.Rdata")

# Last year
# Bind all dfs and calculate average genetic distance
gen_sim_df210_s9 <- melted_df_list_210 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df210_s9, file = "data/s9_gen_sim_df210.Rdata")
