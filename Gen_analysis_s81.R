...
for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
  ...
  gen_out1 <- read.table(paste0("Outputs/Batch81_Sim1_Land1_Rep", replicate, "_Genetics.txt"), header = TRUE)
  # merge
...
t<-read.table("Outputs/Batch81_Sim1_Land1_Rep0_Inds.txt")
# Loop through each replicate
for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
  # Read in genetic output
  ind_out1 <- read.table(paste0("Outputs/Batch81_Sim1_Land1_Rep", replicate, "_Inds.txt"), header = TRUE)
  # merge
...
gen_ind_out_s81 <- gen_out %>%
  left_join(dplyr::select(ind_out, IndID, Rep, Year, PatchID, Natal_patch), by = c("IndID", "Year", "Rep"))
# save
gen_ind_out_s81[121:240] <- NULL
save(gen_ind_out_s81, file = "data/gen_ind_out_s81.RData")
...
# Load data
load("data/gen_ind_out_s81.RData")

# subset data and drop all rows with PatchID = 0 (currently not in a population)
filtered_data_s81 <- gen_ind_out_s81[!(gen_ind_out_s81$PatchID==0),]

## Splitting by by Year + replicate number
# Initialize a list to store filtered data for each year and replicate
filtered_data_list <- list()

# Print unique values of Year and Rep in filtered_data_s81
cat("Unique Years in filtered_data_s81:", unique(filtered_data_s81$Year), "\n")
cat("Unique Replicates in filtered_data_s81:", unique(filtered_data_s81$Rep), "\n")
...
for (year in c(20, 210)) {
  for (rep in 0:(num_replicates - 1)) { # replicates 0-9
    ...
    filtered_data_year_rep <- filtered_data_s81[!is.na(filtered_data_s81$Year) & 
                                                  !is.na(filtered_data_s81$Rep) &
                                                  filtered_data_s81$Year == year & 
                                                  filtered_data_s81$Rep == rep, ]
...
for (rep in 0:(num_replicates - 1)) {
  # Get filtered data for the current replicate
  filtered_data_rep <- filtered_data_list[[paste0("filtered_data20_", rep)]]
  
  # 1. Confirm dimensions
  cat("Dimensions of filtered_data_rep:", dim(filtered_data_rep), "\n")
  
  # 2. Check for missing values and non-character entries
  if (any(is.na(filtered_data_rep))) {
    print("There are missing values in filtered_data_rep.")
  }
  if (!all(sapply(filtered_data_rep, is.character))) {
    print("There are non-character entries in filtered_data_rep.")
  }
  
  # 3. Verify alleles separation
  # Check a sample of the data to verify if alleles are properly separated
  sample_alleles <- head(as.vector(unlist(filtered_data_rep[, 5:120])), 10)  # Extract a sample of alleles
  cat("Sample of alleles:", sample_alleles, "\n")
}
#----------------------------------------------------
...
for (rep in 0:(num_replicates - 1)) { # replicates 0-99
  ...
  tryCatch({
    ...
    # Store gen_data object in the list
    gen_data_list_210[[paste0("gen_data20_", rep)]] <- gen_data
  }, error = function(e) {
    cat("An error occurred in replicate", rep, ": ", conditionMessage(e), "\n")
    # Skip to the next iteration
    next
...
gen_sim_df20_s81 <- melted_df_list_20 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df20_s81, file = "data/s81_gen_sim_df20.Rdata")

# Last year
# Bind all dfs and calculate average genetic distance
gen_sim_df210_s81 <- melted_df_list_210 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df210_s81, file = "data/s81_gen_sim_df210.Rdata")
