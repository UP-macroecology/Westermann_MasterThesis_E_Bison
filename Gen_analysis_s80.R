...
for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
  ...
  gen_out1 <- read.table(paste0("Outputs/Batch80_Sim1_Land1_Rep", replicate, "_Genetics.txt"), header = TRUE)
  # merge
...
t<-read.table("Outputs/Batch80_Sim1_Land1_Rep0_Inds.txt")
# Loop through each replicate
for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
  # Read in genetic output
  ind_out1 <- read.table(paste0("Outputs/Batch80_Sim1_Land1_Rep", replicate, "_Inds.txt"), header = TRUE)
  # merge
...
gen_ind_out_s80 <- gen_out %>%
  left_join(dplyr::select(ind_out, IndID, Rep, Year, PatchID, Natal_patch), by = c("IndID", "Year", "Rep"))
# save
gen_ind_out_s80[121:240] <- NULL
save(gen_ind_out_s80, file = "data/gen_ind_out_s80.RData")
...
# Load data
load("data/gen_ind_out_s80.RData")

# subset data and drop all rows with PatchID = 0 (currently not in a population)
filtered_data_s80 <- gen_ind_out_s80[!(gen_ind_out_s80$PatchID==0),]

## Splitting by by Year + replicate number
# Initialize a list to store filtered data for each year and replicate
filtered_data_list <- list()

# Print unique values of Year and Rep in filtered_data_s80
cat("Unique Years in filtered_data_s80:", unique(filtered_data_s80$Year), "\n")
cat("Unique Replicates in filtered_data_s80:", unique(filtered_data_s80$Rep), "\n")
...
for (year in c(20, 210)) {
  for (rep in 0:(num_replicates - 1)) { # replicates 0-9
    ...
    filtered_data_year_rep <- filtered_data_s80[!is.na(filtered_data_s80$Year) & 
                                                  !is.na(filtered_data_s80$Rep) &
                                                  filtered_data_s80$Year == year & 
                                                  filtered_data_s80$Rep == rep, ]
...
gen_sim_df20_s80 <- melted_df_list_20 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df20_s80, file = "data/s80_gen_sim_df20.Rdata")

# Last year
# Bind all dfs and calculate average genetic distance
gen_sim_df210_s80 <- melted_df_list_210 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df210_s80, file = "data/s80_gen_sim_df210.Rdata")
