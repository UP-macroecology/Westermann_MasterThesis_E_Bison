...
#for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
 ...
 # gen_out1 <- read.table(paste0("Outputs/Batch66_Sim1_Land1_Rep", replicate, "_Genetics.txt"), header = TRUE)
  # merge
...
#t<-read.table("Outputs/Batch66_Sim1_Land1_Rep0_Inds.txt")
# Loop through each replicate
#for (replicate in 0:(num_replicates - 1)) { # replicates 0-9
  # Read in genetic output
 # ind_out1 <- read.table(paste0("Outputs/Batch66_Sim1_Land1_Rep", replicate, "_Inds.txt"), header = TRUE)
  # merge
...
# Load data
load("data/gen_ind_out_s66.RData")
gen_ind_out_s66[121:240] <- NULL
save(gen_ind_out_s66, file = "data/gen_ind_out_s66.RData")
# subset data and drop all rows with PatchID = 0 (currently not in a population)
filtered_data_s66 <- gen_ind_out_s66[!(gen_ind_out_s66$PatchID==0),]
...
for (year in c(20, 210)) {
  for (rep in 0:(num_replicates - 1)) { # replicates 0-9
    ...
    filtered_data_year_rep <- filtered_data_s66[!is.na(filtered_data_s66$Year) & 
                                                  !is.na(filtered_data_s66$Rep) &
                                                  filtered_data_s66$Year == year & 
                                                  filtered_data_s66$Rep == rep, ]
...
gen_sim_df20_s66 <- melted_df_list_20 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df20_s66, file = "data/s66_gen_sim_df20.Rdata")

# Last year
# Bind all dfs and calculate average genetic distance
gen_sim_df210_s66 <- melted_df_list_210 %>%
  bind_rows() %>% # Combine to single df
  group_by(Population1, Population2) %>%
  summarize(Avg_Gen_sim = mean(Gen_Similarity))
# save
save(gen_sim_df210_s66, file = "data/s66_gen_sim_df210.Rdata")
